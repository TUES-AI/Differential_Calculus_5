...
\begin{tikzpicture}[scale=1.4]
  ...
  % Contour circles
  \foreach \r/\lbl in {0.6/c_1,1.2/c_2,1.8/c_3} {
    \draw[gray!70, thick] (0,0) circle (\r);
  }
  \node[gray] at (0.4,-0.8) {$c_1$};
  \node[gray] at (0.8,-1.4) {$c_2$};
  \node[gray] at (1.2,-2.0) {$c_3$};
  % Gradient arrow
  \draw[-{Stealth[length=3mm]},thick,blue] (0.6,0.3) -- (1.6,0.8)
    node[above] {$\grad f$};
  % Tangent to contour
  \draw[thick,red] (0.9,0.82) -- (0.3,1.15);
  \node[red, right] at (0.9,1.0) {допирателна};
  % Right angle marker
  \draw[blue] (0.72,0.54) -- (0.62,0.62) -- (0.70,0.72);
\end{tikzpicture}
\caption{Ниви линии и градиент: $\grad f$ сочи в посоката на най-бързо
нарастване и е \emph{перпендикулярен} на нивата.}
\end{figure}
